...
async function requestDeepSeek(prompt, env, validator = validateAnalysis) {
  ...
  const analysis = validator(parseJsonContent(data?.choices?.[0]?.message?.content));
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    if (body?.scale === "generic") {
      const payload = parseGenericRequest(body);
      const analysis = await requestDeepSeek(buildGenericPrompt(payload), env, validateGenericAnalysis);
